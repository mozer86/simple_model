...
impl EnumObject{
            ...
            match variant {
                Variant::Unnamed(s)=>{
                    ...
                    for field in s.fields.iter() {
                        ...
                        ret += &format!("{}", field.get_documentation_type());
                        i+=1;
                    }
                    ret += &format!(")\n```\n\n");
                },
    ...
    pub fn gen_group_api(&self)->TokenStream2{
        ...
        let location_str = crate::object_location(name_str).expect(&format!("Trying to gen a Group API of '{}', which is not registered in Model", &object_name));
